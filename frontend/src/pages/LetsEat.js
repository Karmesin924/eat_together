...
const LetsEat = () => {
  ...
  const { people, gender, age, menu, conversation, handleSaveFilters } = useContext(MyContext);
  ...
  const handleMatching = () => {
    ...
    axios
      ...
      .then((response) => {
        // 필터 선택 값들을 성공적으로 백엔드로 전송한 후 수행할 작업
          ...
          <div className="flex flex-col justify-center items-center m-auto border-4 border-project rounded-xl w-1/2 mt-5">
            ...
            <div className="text-xl pb-3">
              ...
              <p>
                ...
                <span className=" text-red-500 font-semibold">{menu.join(', ')}</span>
              </p>
              <p>
                <span className="font-bold">대화빈도: </span>
                <span className=" text-red-500 font-semibold">{conversation}</span>
              </p>
            </div>
          </div>
          <div className="flex flex-col justify-center items-center m-auto border-4 border-project rounded-xl w-1/2 mt-5">
            <p className="font-bold text-3xl pt-3 pb-3">현재 위치</p>
            <p className=" text-gray-500 pb-3">※현재 위치 기준으로 근처 인원이 매칭됩니다 :)</p>
            {latitude && longitude ? (
              <div>
                <p>
                  위도: {latitude}, 경도: {longitude},{' '}
                </p>
                {mapLoaded ? <Map latitude={latitude} longitude={longitude} /> : <p>지도를 로딩 중입니다...</p>}
              </div>
            ) : (
              <p>위치 정보를 가져오는 중...</p>
            )}
