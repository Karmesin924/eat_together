...
const SignUp = () => {
  ...
  const checkButtonEnabled = useCallback(() => {
    if (isName && isEmail && isBirth && isNickname && isPassword && isPasswordConfirm && gender !== '') {
      setIsButtonEnabled(true);
    } else {
      setIsButtonEnabled(false);
    }
  }, [isName, isEmail, isBirth, isNickname, isPassword, isPasswordConfirm, gender]);
    ...
    <div className="signup">
      ...
      <div className="p-5 m-0">
        <div className="flex flex-col gap-4 max-w-md mt-0 m-auto">
          ...
          />
          <span className="text-sm font-semibold text-signupmessage">{nameMessage}</span>
          ...
          />
          <span className="text-sm font-semibold text-signupmessage">{emailMessage}</span>
          ...
          <div className="font-semibold text-xl flex gap-4">
            <label htmlFor="male">
              <input type="radio" id="male" value="man" checked={gender === 'man'} onChange={onChangeGender} />
              남성
            </label>
            <label htmlFor="female">
              <input type="radio" id="female" value="woman" checked={gender === 'woman'} onChange={onChangeGender} />
              여성
            </label>
          </div>
          ...
          />
          <span className="text-sm font-semibold text-signupmessage">{birthMessage}</span>
          ...
          />
          <span className="text-sm font-semibold text-signupmessage">{nicknameMessage}</span>
          ...
          />
          <span className="text-sm font-semibold text-signupmessage">{passwordMessage}</span>
          ...
          />
          <span className="text-sm font-semibold text-signupmessage">{passwordConfirmMessage}</span>

          <MyButton text="가입하기" onClick={onClickSignUp} disabled={!isButtonEnabled} />
        </div>
      </div>
    </div>
  );
